    ...
    <div className="container mx-auto max-w-7xl py-12 px-4 md:px-6">
      <div className="mb-8">
       ...
       <div className="mb-8">
        ...
        {category && <Badge variant="secondary" className="text-lg py-1 px-4">{category}</Badge>}
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-8">
          {results.map((business) => (
            <Card key={business.id} className="flex flex-col md:flex-row overflow-hidden hover:shadow-xl transition-shadow duration-300">
                ...
                <CardHeader>
                  <CardTitle className="text-2xl font-headline">
                   ...
                   <div className="flex items-center pt-2 text-yellow-500">
                      ...
                      {business.rating % 1 !== 0 && <Star className="w-5 h-5 fill-current" style={{ clipPath: 'inset(0 50% 0 0)' }} />}
                      {[...Array(5 - Math.ceil(business.rating))].map((_, i) => (
                         <Star key={i} className="w-5 h-5 text-gray-300" />
                      ))}
                      <span className="ml-2 text-sm text-muted-foreground">({business.reviewCount} сэтгэгдэл)</span>
                    </div>
                </CardHeader>
        ...
        <div className="text-center py-20">
          ...
          <p className="text-muted-foreground mt-2">Хайлтын шалгуураа өөрчлөөд дахин оролдоно уу.</p>
        </div>
      )}
    </div>
  );
