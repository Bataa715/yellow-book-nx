            ...
            <Card className="overflow-hidden shadow-xl">
                 <CardHeader className="p-0">
                   <div className="relative h-64 md:h-96">
                        ...
                        <div className="absolute bottom-0 left-0 p-6 md:p-8">
                            <CardTitle className="text-4xl md:text-5xl font-headline text-white drop-shadow-lg">{business.name}</CardTitle>
                             <div className="flex items-center pt-2 text-yellow-400">
                                {[...Array(Math.floor(business.rating))].map((_, i) => (
                                    <Star key={i} className="w-6 h-6 fill-current" />
                                ))}
                                {business.rating % 1 !== 0 && <Star className="w-6 h-6 fill-current" style={{ clipPath: 'inset(0 50% 0 0)' }} />}
                                {[...Array(5 - Math.ceil(business.rating))].map((_, i) => (
                                    <Star key={i} className="w-6 h-6 text-gray-300 fill-current" />
                                ))}
                                <span className="ml-2 text-lg font-medium text-white">({business.reviewCount} сэтгэгдэл)</span>
                            </div>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="p-6 md:p-8 grid md:grid-cols-3 gap-8">
                    <div className="md:col-span-2 space-y-8">
                                    ...
                                    <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3">
                                        ...
                                        <Card>
                                            <CardContent className="relative flex aspect-video items-center justify-center p-0 overflow-hidden rounded-lg">
                                                 <Image src={img} alt={`${business.name} image ${index + 1}`} fill className="object-cover" />
                                            </CardContent>
                                        </Card>
                                        </div>
                                    </CarouselItem>
                        ...
                        <div>
                            <h3 className="text-2xl font-bold mb-4 border-b pb-2 flex items-center"><MessageSquare className="mr-3"/>Сэтгэгдлүүд</h3>
                    ...
                    <div className="space-y-6">
                            ...
                            <CardContent className="space-y-4 text-sm">
                                ...
                                {business.contact.website && (
                                    <div className="flex items-center">
                                        ...
                                        <a href={business.contact.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline break-all">
                                            {business.contact.website}
                                        </a>
                                    </div>
                                )}
                        ...
                        <Card>
                             <CardHeader>
                                <CardTitle className="flex items-center"><Clock className="mr-3"/>Цагийн хуваарь</CardTitle>
                            </CardHeader>
                            <CardContent>
